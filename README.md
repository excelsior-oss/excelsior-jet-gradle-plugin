...
the plugin dependency to the `buildscript{}` configuration
of your `build.gradle` file first:
...
a zip archive named `<artifactName>.zip`
...
of the Gradle build script that a particular dependency is a third party library
...
into the `excelsiorJet{}` plugin extension:
...
that has the following parameters:
  ...
  * `version = `*`version-string`*
    ...
    **Notice:** unlike Gradle `project.version`, this string
...
in the `jet` subdirectory of the Gradle target build directory.
You may configure its name with the `detachedPackage` parameter
of the `slimDown{}` section
(by default the name is `artifactName.pkl`).
...
Gradle build script.
For that, add the following dependency to your `build.gradle` file:
...
you need to add the plugin dependency to the `buildscript{}` configuration
